...
class DecisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if self.state_dim is not None:
            self.embed_state_vector = torch.nn.Linear(self.state_dim[0], hidden_size)
        ...
        self.predict_pov = lambda x: x #TODO! make this an option
    ...
    def forward(self, pov, actions, rewards, returns_to_go,timesteps,state_vector=None, attention_mask=None):#TODO maybe call diferent methods for the diferetn options from this forward for redability
        ...
        if self.kmeans_centroids is not None:
            actions = torch.argmin(torch.cdist(actions,self.kmeans_centroids),2) 
        if self.natureCNN:
            pov = pov.reshape(-1, 3, pov.shape[2], pov.shape[3]).type(torch.float32).contiguous() #reshapes the steps as batches (batch * block_size, n_embd)
        ...
        if state_vector is not None:   
            state_embeddings  = self.embed_state_vector(state_vector)
        if self.natureCNN:
    ...
    def get_action(self, states_pov, actions, rewards, returns_to_go, timesteps,states_vector=None, **kwargs):
        ...
        states_pov = states_pov.reshape((1, -1,)+self.pov_dim)
        if states_vector is not None:
            states_vector = states_vector.reshape((1, -1,)+self.state_dim)
        actions = actions.reshape(1, -1, self.act_dim)
        returns_to_go = returns_to_go.reshape(1, -1, 1)
        timesteps = timesteps.reshape(1, -1)

        if self.max_length is not None:
            states_pov = states_pov[:,-self.max_length:]
            if states_vector is not None:
                states_vector =states_vector[:,-self.max_length:]       
            actions = actions[:,-self.max_length:]
            returns_to_go = returns_to_go[:,-self.max_length:]
            timesteps = timesteps[:,-self.max_length:]
            
            # pad all tokens to sequence length
            attention_mask = torch.cat([torch.zeros(self.max_length-states_pov.shape[1]), torch.ones(states_pov.shape[1])])
            attention_mask = attention_mask.to(dtype=torch.long, device=states_pov.device).reshape(1, -1)
            states_pov = torch.cat(
                    [torch.zeros((states_pov.shape[0], self.max_length-states_pov.shape[1], )+self.pov_dim, device=states_pov.device), states_pov],
                    dim=1).to(dtype=torch.float32)
            if states_vector is not None:
                states_vector = torch.cat(
                    [torch.zeros((states_vector.shape[0], self.max_length-states_vector.shape[1], )+self.state_dim, device=states_vector.device), states_vector],
                    dim=1).to(dtype=torch.float32)
            actions = torch.cat(
        ...
        _,_, action_preds, return_preds = self.forward(
            states_pov, actions, None, returns_to_go, timesteps,state_vector=states_vector, attention_mask=attention_mask, **kwargs)
