...
def main(parameters
        ...
        dataset_rewards2go=calculate_rewards2go(trajectories)#We precalculate all the rewards2go for the dataset
        '''
        trajectories= []
        for name in trajectory_names:
                trajectories.append(data.load_data(name, skip_interval=0, include_metadata=False))
        ...
        def get_batch(batch_size=64, max_len=max_length):
                ...
                for i in range(batch_size):
                        trajectory_index =random.randint(0,len(trajectories)-1)
                        trajectory = trajectories[trajectory_index]#TODO entender codigo paper dt de selecionar trayectories
                        trajectory=list(trajectory)#load the trajectory from generator
                        inicio = random.randint(0, len(trajectory) - 1)
                        ...
                        timesteps.append(range(inicio,inicio+max_length))
        ...
        if log_to_wandb:
                wandb.init(
            name=random.randint(int(1e5), int(1e6) - 1),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_ep_len', type=int, default=18000)#default of the 
    ...
    parser.add_argument('--checkpoint_name', type=str, default="decisiontransformers_modified_8x8x1vaeq_256_noKmeans")#TODO change
